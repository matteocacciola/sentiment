...
describe('getAnalysisResults', () => {
  ...
  it('should return the expected sentiment stats and scores', async () => {
    ...
    const result = await getAnalysisResults(company, items, strategyType, scoreThreshold);
    expect(result).toEqual(expectedResult);
  });

  it('should call strategyProvider with the correct strategy type', async () => {
    await getAnalysisResults(company, items, strategyType, scoreThreshold);
  ...
  it('should throw an error if company argument is missing', async () => {
    await expect(getAnalysisResults(undefined as unknown as string, items, strategyType, scoreThreshold))
  ...
  it.each([undefined, []])('should return null if items argument is missing or empty', async (item) => {
    expect(await getAnalysisResults(company, item as unknown as string[], strategyType, scoreThreshold))
      .toBeNull();
  });
});
